#!/usr/bin/env python 
#==============================================================================================
# Code to fix the FEED and the FIELD tables in ASKAP msdata 
# for use with external softwares -- like CASA. 
#
# Note: 
#      1. This code is designed for msdata that was recorded with fringe-tracking per beam 
#      2. The fringe-tracking per beam was introduced towards Nov 2018. An easy way to check 
#         if this code would work with your ms is to ensure that the "BEAM OFFSET" column in 
#         the FEED table have non-zero values. 
#      3. Be aware that the visibility scales in ASKAP is a factor of 2 off from what CASA 
#         and other software assumes -- a difference owing to the definition of Stokes-params. 
#
# This code was developed and made avilable by Emil Lenc. 
#==============================================================================================
...
if tableexists("%s/FIELD_BACKUP" %(ms)) == False:
    print("Making backup of original FIELD table")
    tablecopy(tablename="%s/FIELD" %(ms), newtablename="%s/FIELD_BACKUP" %(ms))
else:
    print("Original copy of FIELD table is being used")

if tableexists("%s/FEED_BACKUP" %(ms)) == False:
    print("Making backup of original FEED table")
    tablecopy(tablename="%s/FEED" %(ms), newtablename="%s/FEED_BACKUP" %(ms))
else:
    print("Original copy of FEED table is being used")

print("Reading phase directions")
tp = table("%s/FIELD_BACKUP" %(ms), readonly=True, ack=False)
...
tf = table("%s/FEED_BACKUP" %(ms), readonly=True, ack=False)
