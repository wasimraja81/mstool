...
def write_correction_table(
    ...
    # Each comment line is written as a quoted CSV field so that spreadsheet
    # viewers (Numbers, Excel) treat the entire line as a single cell in column A
    # rather than splitting on embedded commas.
    def _c(s: str) -> str:
        """Return *s* as a double-quoted CSV field followed by a newline.
        Any embedded double-quotes are escaped per RFC 4180 ('' doubling)."""
        return '"' + s.replace('"', '""') + '"\n'

    with csv_path_out.open("w", newline="") as fh:
        # ── self-documenting header (pandas skips lines starting with '#') ──
        fh.write(_c("# dQ/dU per-beam mean correction factors"))
        fh.write(_c("# Generated by: plot_dQ_vs_beam.py  (mstool/projects/calibration-updates-2026)"))
        fh.write(_c(f"# Generated  : {datetime.datetime.now().strftime('%Y-%m-%d %H:%M')}"))
        fh.write(_c(f"# Source     : {meta['csv_path']}"))
        fh.write(_c(f"# Selection  : manifest indices {meta['start_index']}\u2013{meta['end_index']},"
                    f" excl {meta['exclude_indices']!r}"))
        fh.write(_c(f"# SB_REFs    : {meta['n_sbrefs']} contributing observations"))
        fh.write(_c(f"# Footprint  : {meta.get('footprint_name', 'unknown')}"))
        fh.write(_c("#"))
        fh.write(_c("# Columns:"))
        fh.write(_c("#   footprint   beam footprint name (from schedblock metadata, e.g. closepack36)"))
        fh.write(_c("#   field       reference field name (e.g. REF_1324-28)"))
        fh.write(_c("#   variant     calibration variant: 'bpcal' (bandpass cal) or 'lcal' (leakage cal)"))
        fh.write(_c("#               note: footprint is constant across all rows in a single-manifest run;"))
        fh.write(_c("#               rows from different footprints (different manifests) can be safely appended"))
        fh.write(_c("#   beam        ASKAP beam index, 0-based (0-35 for closepack36)"))
        fh.write(_c("#   mean_dQ     mean fractional Stokes-Q leakage dQ/I (%), signed, averaged"))
        fh.write(_c("#               across all SB_REF x ODC observations for this field/variant/beam"))
        fh.write(_c("#   std_dQ      standard deviation of dQ/I (%) across observations"))
        if has_dU:
            fh.write(_c("#   mean_dU     mean fractional Stokes-U leakage dU/I (%), signed"))
            fh.write(_c("#   std_dU      standard deviation of dU/I (%) across observations"))
        fh.write(_c("#   n_obs       number of (SB_REF, ODC) data rows averaged for this beam"))
        fh.write(_c("#"))
        fh.write(_c("# Physical meaning:"))
        fh.write(_c("#   dQ = (Q_measured / I_measured) x 100  -- on-axis Stokes-Q leakage in percent"))
        fh.write(_c("#   dU = (U_measured / I_measured) x 100  -- on-axis Stokes-U leakage in percent"))
        fh.write(_c("#   For an unpolarised calibrator (1934-638), any measured Q or U is pure leakage."))
        fh.write(_c("#   A positive mean_dQ means the X-feed is systematically stronger than the Y-feed"))
        fh.write(_c("#   for that beam (see gain_calibration_strategy.html for the full derivation)."))
        fh.write(_c("#"))
        fh.write(_c("# Usage (pandas) -- the comment='#' argument skips these header lines:"))
        fh.write(_c("#   import pandas as pd"))
        fh.write(_c("#   df = pd.read_csv('dq_du_correction_factors.csv', comment='#')"))
        fh.write(_c("#   row = df[(df.footprint == 'closepack36') & (df.field == 'REF_1324-28')"))
        fh.write(_c("#            & (df.variant == 'bpcal') & (df.beam == 12)].iloc[0]"))
        fh.write(_c("#   dq, dq_std = row.mean_dQ, row.std_dQ"))
        if has_dU:
            fh.write(_c("#   du, du_std = row.mean_dU, row.std_dU"))
        fh.write(_c("#"))
        fh.write(_c("# To get all 36 beams for a footprint/field/variant as a numpy array:"))
        fh.write(_c("#   sub = df[(df.footprint == 'closepack36') & (df.field == 'REF_1324-28')"))
        fh.write(_c("#            & (df.variant == 'bpcal')].sort_values('beam')"))
        fh.write(_c("#   dq_array = sub.mean_dQ.to_numpy()   # shape (36,), index = beam number"))
        fh.write(_c("#"))
        fh.write(_c("# See also: dq_du_correction_factors.txt  (fixed-width ASCII, greppable)"))
        fh.write(_c("#           gain_calibration_strategy.html (derivation and application)"))
